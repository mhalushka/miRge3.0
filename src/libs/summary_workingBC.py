    ...
    if args.bam_out:
        ...
        with open(mirna_samFile) as miSam:
            ...
            for files_sam in base_names:
                ...
                samcreation = subprocess.run(str(samCom2bam), shell=True)
                ...
                bamsorting = subprocess.run(str(bamsort), shell=True)
                ...
                bamindexing = subprocess.run(str(bamindex), shell=True)
...
def summarize(args, workDir, ref_db,base_names, pdMapped, sampleReadCounts, trimmedReadCounts, trimmedReadCountsUnique):
    ...
    if args.tRNA_frag:
        ...
        bwtExec = bwtCommand+" -a 200000 -e "+ str(file_pre_tRNA)
        ...
        if bowtie.returncode==0:
            ...
            for srow in bwtOut2.split('\n'):
                if '>' in srow:
                    ...
                    pretrnaNameSeqDic.update({srow:str(srow)})
        else:
            allFiles_inPlace = 0
